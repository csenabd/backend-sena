...
class ProyectoSenoService {
  constructor() {
    this.model = proyectoSenoSchema;
  }
  ...
  async update(id, values) {
    const proyecto = await this.model.findByIdAndUpdate(id, values, { new: true });
    return proyecto;
  }
  ...
  async updateObjetivoEspecifico(id, objetivoId, finalizado, resultado) {
    const proyecto = await this.model.findOneAndUpdate(
      ...
      { 
        $set: { 
          "objetivosEspecificos.$.finalizado": finalizado, 
          "objetivosEspecificos.$.resultado": resultado 
        } 
      },
      { new: true }
    );
...
module.exports = ProyectoSenoService;
